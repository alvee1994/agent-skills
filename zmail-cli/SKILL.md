...
- Mutating ones (avoid unless explicitly asked): `send`, `reply`, `delete`, `move`, `archive`, `unarchive`, `spam`, `unspam`, `flag`, `apply-label`, `remove-label`, `remove-all-label`, `read`, `unread`, `upload-attachment`.
  ...
  - `message upload-attachment --account=<id> --upload-type=multipart --attach=<local file path> --file-name-q=<name> -f=JSON` — verified working, but **both flags are load-bearing and undocumented as such**. `--upload-type=multipart` is required, or the file uploads as 0 bytes even though the API accepts the call. `--file-name-q` (not `--file-name`) is the one that actually sets the name — `--file-name` alone errors with `fileName is null`. Success returns `{"status":{"code":200,...},"data":[{"storeName":"...","attachmentName":"...","attachmentPath":"...","url":null}]}`. Note the `attachmentName` in that response reflects the local source filename, not `--file-name-q` — don't rely on it for the display name recipients see. Instead, feed `storeName` and `attachmentPath` from the response into `send --attachments=<display-name>::<attachmentPath>::<storeName>`, choosing `<display-name>` yourself (see the `--attachments` syntax note above — it is not JSON despite the help text).
  - Observed once: `upload-attachment` failed with `An error occurred while doing the operation` / `show-last-error` → `Unable to decrypt data, Check you password`, on an otherwise-valid session (auth still checked out fine, path wasn't the issue — same failure on both a long scratch path and a short one). Re-running the identical command in a **freshly unlocked tmux session** succeeded immediately. If this error shows up, don't loop retries in the same session — kill it and start a new one before trying again.
